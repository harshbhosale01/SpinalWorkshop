...
class AxiLite4WavePlayer(wavePlayerGenerics : WavePlayerGenerics) extends Component {
  ...
  val io = new Bundle {
    val axiLite = slave(AxiLite4(addressWidth = 2*sampleCountLog2, dataWidth = 32)) //2*sampleCountLog2 would allow, eventually, to map the ROM into the memory space (optional)
    val wave = out(Sample)
  }
